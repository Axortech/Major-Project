...
interface ApiProduct {
  ...
  aspect_sentiment_counts: string[];
  image?: string;
  price?: number;
}
...
export const useProductSearch = () => {
    ...
    debounce(async (searchQuery: string) => {
      ...
      try {
        ...
        const response = await axios.post<ApiResponse>(
          API_BASE_URL,
          { input: encodedQuery },
          { cancelToken: cancelTokenSource.current.token, timeout: 300000 },
        );
  ...
  const transformProduct = (product: ApiProduct): SearchResult => ({
    ...
    extractedAspects: Array.isArray(product.extracted_aspects)
      ? product.extracted_aspects
      : [],
    aspect_sentiment_counts: Array.isArray(product.aspect_sentiment_counts)
  ...
  const handleAxiosError = (err: unknown) => {
    ...
    if (axiosError.response) {
      switch (axiosError.response.status) {
        ...
        default:
          setError(
            `${ERROR_MESSAGES.UNEXPECTED_ERROR}: ${axiosError.response.status}`
          );
      }
    } else {
